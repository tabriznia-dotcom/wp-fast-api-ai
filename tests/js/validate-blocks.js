...
function walk(list, file, depth=0) {
  for (const b of list) {
    ...
    if (!b.isValid) {
      ...
      const issues = (b.validationIssues||[]).map(i => i.args ? i.args.map(a => typeof a === 'string' ? a.slice(0,600) : JSON.stringify(a).slice(0,600)).join(' | ') : String(i)).join('\n');
...
console.log(failures ? `FAILED: ${failures} invalid blocks` : `OK: all blocks valid in ${files.length} files`);
