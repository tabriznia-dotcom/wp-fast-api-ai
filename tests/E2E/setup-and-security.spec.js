...
test.describe( 'Settings and security', () => {
	test( 'provider setup never exposes the API key', async ( { page } ) => {
		...
		const status = await page.evaluate( () =>
			window.wp.apiFetch( { path: '/aipd/v1/status' } )
		);
	...
	test( 'subscribers cannot access plugin screens or REST routes', async ( {
		browser,
		baseURL,
	} ) => {
		...
		const response = await page.request.get(
			`${ baseURL }/?rest_route=/aipd/v1/status`
		);
		expect( response.status() ).toBe( 401 );
		await page.goto(
			`${ baseURL }/wp-admin/admin.php?page=aipd-providers`
		);
