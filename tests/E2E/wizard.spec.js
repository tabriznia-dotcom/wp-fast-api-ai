...
async function runWizard( page, language ) {
	...
	await expect(
		page.getByRole( 'heading', { name: 'Business and audience', level: 2 } )
	).toBeFocused();

	await page
		.getByLabel( 'Topic (required)' )
		.fill( 'Online cooking classes' );
	await page.getByLabel( 'Main call to action' ).fill( 'Book a class' );
	await page.getByRole( 'button', { name: 'Continue' } ).click();

	await page
		.getByLabel( 'Content language' )
	...
	await expect(
		page.getByRole( 'dialog', { name: 'Send this request?' } )
	).toBeVisible();
	await page.getByRole( 'button', { name: 'Send request' } ).click();
	await expect( page.getByLabel( 'Section name' ).first() ).toBeVisible();
	await page
		.getByRole( 'button', { name: /Remove FAQ|Remove پرسش‌ها/ } )
		.click();
	await page
		.getByRole( 'button', { name: 'Approve and write content' } )
		.click();
	await page.getByRole( 'button', { name: 'Send request' } ).click();
	...
	await expect(
		page
			...
			.getByText( 'Section updated in the preview.' )
	).toBeVisible();
...
test.describe( 'Page wizard', () => {
	...
	test( 'English LTR landing page: wizard, editor and frontend', async ( {
		page,
	} ) => {
		const editUrl = await runWizard( page, 'English' );
		await expectValidBlocks( page, editUrl );

		const postId = new URL( editUrl, 'http://x' ).searchParams.get(
			'post'
		);
		for ( const vp of VIEWPORTS ) {
			...
			await page.screenshot( {
				path: artifact( `screenshots/landing-en-${ vp.name }.png` ),
				fullPage: true,
			} );
		}
	...
	test( 'Persian RTL landing page: wizard, editor and frontend', async ( {
		page,
	} ) => {
		const editUrl = await runWizard( page, 'Persian (فارسی)' );
		await expectValidBlocks( page, editUrl );
		// The editor canvas shows the same RTL layout as the front end.
		const canvas = page.frameLocator( 'iframe[name="editor-canvas"]' );
		await expect( canvas.locator( '.aipd-page' ).first() ).toBeVisible();
		expect(
			await canvas
				.locator( '.aipd-page' )
				.first()
	...
	test( 'template path works without any AI request and creates a classic draft', async ( {
		page,
	} ) => {
		await page.goto( admin( 'wizard' ) );
		await page
			.getByLabel( 'Start from a template (no AI request)' )
			.check();
		await page
	...
	test( 'wizard warns before leaving with unsaved changes', async ( {
		page,
	} ) => {
		await page.goto( admin( 'wizard' ) );
		await page.getByRole( 'button', { name: 'Continue' } ).click();
	...
	test( 'wizard is keyboard operable and works in an RTL admin', async ( {
		page,
		context,
		baseURL,
	} ) => {
		await context.addCookies( [
			{ name: 'aipd_e2e_rtl', value: '1', url: baseURL },
		] );
		await page.goto( admin( 'wizard' ) );
		await expect( page.locator( '.aipd-wizard' ) ).toHaveAttribute(
			'dir',
			'rtl'
		);
		...
		await expectNoHorizontalOverflow( page );
		await page.screenshot( {
			path: artifact( 'screenshots/wizard-rtl-admin-mobile.png' ),
			fullPage: true,
		} );
...
test( 'generated page has exactly one H1 and text icons', async ( {
	page,
} ) => {
	await page.goto( admin( 'wizard' ) );
